    ...
    if twitter_query:
        ...
        with tab1:
            with st.form("basic_twitter_form"):
                ...
                search_params_basic["limit"] = b.slider("Tweet limit", 1, 1024, 50)

                if query_exists:
                    st.info("We found an existing query. Updating and deleting will be available soon!", icon="ℹ️")
                    
                    col1, col2 = st.columns(2)
                    with col1:
                        update_basic = st.form_submit_button(label="Update", disabled=True)

                    with col2:
                        delete_basic = st.form_submit_button(label="Delete", disabled=True)
                else:
